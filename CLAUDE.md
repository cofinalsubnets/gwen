...
; `make test_slow` as test_proof + test_gen, the latter auto-generated by tools/spec2coq.l). so this
...
;   because a gate whose subject is an app or a tool wakes the baked image ($(mw), test/test.mk)
;   instead of paying a 1.05s egg boot; the EGG gates (test_host, test_love0, test_gcheck, the
;   kernel/wasm/raw corpus runs) stay cold, and so does test_sat, which the image lane slows 4x.
;   `make valg` for memory. one file: `out/host/love test/x.l` -- but the corpus runs CONCATENATED
;   in one global scope, so keep helpers local (give `:` a body), and a single-file run lacks the asserts.
...
;   one-file download door, gated by test_dist (smoke, in test_slow) + test_up (the whole door,
