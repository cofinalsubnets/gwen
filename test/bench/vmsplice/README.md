...
table over the whole corpus — with what it says to build next — is in,
*the auto lane*.
...
LOVE_NO_GLAZE=1 out/host/love \
  -l test/bench/vmsplice/samples.l test/bench/vmsplice/auto.l    # the same, timed
sh test/bench/vmsplice/run.sh           # the original probe (splice.c + the hand-made body.c)
```

All need `make host` first. Results and the argument live in: "the splice client" for the numbers, "the splice JIT
