...
test('expressions', t => {
  ...
  return Promise.all([
    t.test('read', t => assert.deepEqual(l, p.read(s))),
    t.test('show', t => assert.equal(s, p.show(l))),
  ]);
});
...
test('common tests', t => {
  ...
  return Promise.all([ 'church', 'closure', 'heron', 'lambda', 'tak' ].map(n =>
    fs.readFile(`../test/${n}.p`).then(r =>
      t.test(n, x => p.eval([Let, Assert, assert, p.read(n.toString())])))));
});
...
test('fib', t => {
  ...
  assert(p.eval(p.read(prog)) === fib(n));
});
