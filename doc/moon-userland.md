...
| **PDCLib** | 232/233 files, and its own per-function test drivers run with **zero mooncc-only failures** against a gcc-built control. The one refusal is dlmalloc — `<pthread.h>` itself parses now (`__extension__` landed 2026-08-09), and what remains is exactly one row: PDCLib's `assert_667` (`sizeof(sizeof(int)) == sizeof(unsigned long)`, doc/moon-c-gaps' priced sizeof-as-size_t rung) — the bswap trio, the `__sync` spin-lock pair and `__builtin_clz`/`ctz` all landed 2026-08-09, and with that assert hand-neutered dlmalloc compiles whole on x64. Its `%f` is what found the duplicated-lvalue bug (doc/moon.md, `calm?`) — and, first, the trap
that the control has to be the same library built by gcc (doc/moon-c-gaps). ⚠ its 117 `-D`s must be passed as **quoted** argv entries — 35 of them contain a space (`-D__SIZE_TYPE__=long unsigned int`), and a shell that word-splits them hands the compiler a truncated typedef and a stray `int`. |
