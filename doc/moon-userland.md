...
| **PDCLib** | 232/233 files, and its own per-function test drivers run with **zero mooncc-only failures** against a gcc-built control. The one refusal is dlmalloc — `<pthread.h>` itself parses now (`__extension__` landed 2026-08-09), and what remains is its own ladder: PDCLib's `assert_667` (`sizeof(sizeof(int)) == sizeof(unsigned long)`, doc/moon-c-gaps' priced sizeof-as-size_t rung), then glibc's `__builtin_bswap16/32/64` inlines, then `__sync_lock_test_and_set` under its spin locks. Its `%f` is what found the duplicated-lvalue bug (doc/moon.md, `calm?`) — and, first, the trap
