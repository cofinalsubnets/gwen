...
| **PDCLib** | 232/233 files, and its own per-function test drivers run with **zero mooncc-only failures** against a gcc-built control. The one refusal is dlmalloc, which reaches glibc's `<pthread.h>`. Its `%f` is what found the duplicated-lvalue bug (doc/moon.md, `calm?`) — and, first, the trap
