...
| **PDCLib** | 232/233 files, and its own per-function test drivers run with **zero mooncc-only failures** against a gcc-built control. The one refusal is dlmalloc, which reaches glibc's `<pthread.h>`. Its `%f` is what found the read-modify-write lvalue bug (doc/moon-c-gaps). ⚠ its 117 `-D`s must be passed as **quoted** argv entries — 35 of them contain a space (`-D__SIZE_TYPE__=long unsigned int`), and a shell that word-splits them hands the compiler a truncated typedef and a stray `int`. |
