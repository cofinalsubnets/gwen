...
The external tools stay external; the builtins are the ones that must run inside the shell's own process: **cd**, **pwd**, **exit**, **export**, **command**, **read**, **set**, **shift**, **unset**, **eval**, **.**, **wait**, **local**, **break**, **continue**, **return**, **jobs**, **fg**, **bg**, **:**, **true**, **false**, **love**. `return` inside a `.`-sourced file stops that file (the POSIX dot-return), which is how */etc/profile.d* guards bail early.

`love EXPR ...` joins its arguments into one love form, evaluates it in the shell's own image -- lush *is* love, so no process is spawned -- prints the answer, and exits with its truth bit: positive is true, so `love 1 + 2` prints `3` and `if love 3 > 2` takes the then-arm. A string answers its bytes, so `x=$(love '"a b"')` holds `a b`; anything else prints in love notation, so `love '[1 2 3]'` prints `(1 2 3)`. `-n` says nothing and answers the status alone. A binding the form makes outlives the line, since the image is the shell's. An unfinished or raising form is a message on standard error and status 1 -- never a dead shell. QUOTE THE FORM: the shell's own lexer runs first, so `(`, `*`, `"` and `$` reach love only inside quotes. **love** shadows the interpreter for the bare word, as any builtin shadows its PATH twin; a path spelling (`./love`, `/usr/bin/love`) still spawns one.
