...
`autogroup` (love/glaze/auto.l) is the entry the ev-rebind hands every top-level `:`.
...
source. Two facts about that shape, taken together, once turned an ordinary library
into a file whose sibling calls read the live book.
...
## the one that is fixed: dehof left names dangling
...
lib/source.l six bindings left the form (`src-head src-lay-tgz src-lay-love src-lay
...
## the one that is open: swgather swallows by spelling

`swheads` collects call heads syntactically, with no binder set, and `swgather` pulls in
any global `srcrec` recorded under that name. A name bound as a **local** therefore
reaches for a global of the same spelling:
...
group is non-empty and the lowered form is committed.

Nothing miscompiles through this today. What it costs is a cliff: **any top-level name,
anywhere in the session, can switch a file from "compiled as written" to "compiled from
a nine-pass rewrite"**, and which name does it depends on the session's book. A
scope-aware `swheads` closes it.

## the method

The failure looked like a letrec/global-name bug in c0's `ana_v` — a sibling reference
emitted as `lvm_index`. It was not c0 at all: `-l` and `-e` compile through love/ev.l,
...
```sh
LOVE_NO_GLAZE=1 love -l probe.l -l lib/gz.l -l lib/tar.l -l lib/source.l \
