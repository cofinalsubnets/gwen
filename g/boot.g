...
;; functions for working with variable scope records
    ...
    (scop par arg imp) (:
     ...
     t)
;; thread initializer
    (thd0 c n) (p1 g_vm_ret (p0 (arity c) (seek (+ 1 n) (thd (+ 2 n)))))
;; import a variable into this scope
    (import c v) (? (not (toplp c)) (cpush c 'imp v))

    (em_ap k n) (:
     j (k (+ 1 n))
     (? (= (peek j) ret) (p0 g_vm_tap j)
                           (p1 g_vm_ap j)))
    (em_apn n k m) (:
     j (k (+ 2 m))
     (? (= (peek j) g_vm_ret) (p1 g_vm_tapn (p0 n j))
                           (p1 g_vm_apn (p1 n j))))
    (stkidx d x)
      (: imp (impof d)
         i (lidx x imp)
...
(: ; can't use prelude yet directly here
 ...
 e1 (e0 eval)      ; middle eval
 _ (go e1 prelude) ; final prelude
 e2 (e1 eval)      ; final eval
 t1 (clock 0)
