...
static g_vm(sysinfo) {
  ...
  twoini(si + 3, gputnum(Hp - (intptr_t*) f), word(si + 4));
  twoini(si + 4, gputnum((intptr_t*) f + f->len - Sp), g_nil);
  Ip += 1;
  return Continue(); }
...
static g_noinline struct g *g_ana(struct g *f, g_vm_t *y) {
  ...
  if (!g_ok(f)) return f;
  struct env *c = (struct env*) gpop1(f);
  intptr_t x = f->sp[0];
