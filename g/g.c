...
static g_inline int giungetc(struct g*f, struct g_in *i, int c) { return i->ungetc(f, c, i); }
static g_inline int gfeof(struct g*f, struct g_in *i) { return i->eof(f, i); }
...
static g_inline struct g *grunvm(struct g *f) {
#if g_tco
...
static g_inline struct g *geval(struct g *f) {
  return grunvm(gana(f, gvmyieldk)); }
...
static g_vm(gvmjump) { return
  Ip = Ip[1].m,
  Continue(); }

static g_vm(gvmcond) { return
...
static Cata(gcatacurry) {
  ...
  return ar == 1 ?  pull(f, c, m) : gcataix_(f, c, m, gvmcurry, gputnum(ar)) ; }
...
static Ana(analyze) {
  ...
  // application.
  return avec(f, a, f = ganaapargs(f, c, b)),
         analyze(f, c, a); }
...
static struct g *ganaseq(struct g*f, struct env* *c, intptr_t a, intptr_t b) {
  if (!g_ok(f)) return f;
  if (twop(b)) avec(f, a, f = ganaseq(f, c, A(b), B(b)),
...
#define forget() ((f)->root=(mm),f)
  ...
  while (twop(exp) && twop(B(exp))) {
    ...
    if (twop(e) && f->lambda == (struct g_atom*) A(e)) {
      f = gpush(f, 2, d, lam);
      f = gconsl(gconsr(ganalambda(f, c, g_nil, B(e))));
      if (!g_ok(f)) return forget();
      lam = gpop1(f); }
    exp = BB(exp); }
...
g_vm(gvmtabdel) { return
  Sp[2] = !tabp(Sp[1]) ? g_nil : gtabdel(f, (struct g_tab*) Sp[1], Sp[2], Sp[0]),
  Sp += 2,
  Ip += 1,
  Continue(); }

g_vm(gvmtablen) { return
...
static g_vm(gvmnomp) { return
  Sp[0] = nomp(Sp[0]) ? gputnum(-1) : g_nil,
  Ip += 1,
  Continue(); }

static g_vm(gvmtabp) { return
  Sp[0] = tabp(Sp[0]) ? gputnum(-1) : g_nil,
  Ip += 1,
  Continue(); }

static g_vm(gvmslen) { return
...
static g_vm(gvmstringp) { return
  Sp[0] = g_strp(Sp[0]) ? gputnum(-1) : g_nil,
  Ip += 1,
  Continue(); }

static g_vm(gvmcar) { return
  Sp[0] = twop(Sp[0]) ? A(Sp[0]) : Sp[0],
  Ip++,
  Continue(); }
static g_vm(gvmcdr) { return
...
static int gvfprintf(struct g*f, struct g_out*o, const char *fmt, va_list xs) {
 int c;
 while ((c = *fmt++)) {
  if (c != '%') gfputc(f, c, o);
