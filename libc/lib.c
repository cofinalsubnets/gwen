...
const char * _PDCLIB_strtox_prelim( const char * p, char * sign, int * base )
{
    ...
    if ( *p == '0' )
    {
        ...
        if ( ( *base == 0 || *base == 16 ) && ( *p == 'x' || *p == 'X' ) )
        {
            ...
            */
            if ( memchr( digits, tolower( (unsigned char)*p ), *base ) == NULL )
            {
                p -= 2;
            }
        }
...
uintmax_t _PDCLIB_strtox_main( const char ** p, unsigned int base, uintmax_t error, uintmax_t limval, int limdigit, char * sign )
{
    ...
    while ( ( x = (const char *)memchr( digits, tolower( (unsigned char)**p ), base ) ) != NULL )
    {
        digit = x - digits;
        ...
        else
        {
            ...
            while ( memchr( digits, tolower( (unsigned char)**p ), base ) != NULL )
            {
                ++( *p );
            }
